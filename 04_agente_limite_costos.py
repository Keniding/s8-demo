...
async def main():
    ...
    try:
        async for evento in runner.run_async(user_id="u1", session_id=sesion.id, new_message=contenido, run_config=RunConfig(max_llm_calls=1)):
            for parte in (evento.content.parts if evento.content else []):
                if getattr(parte, "function_call", None):
                    print("1ª llamada al modelo, permitida:", parte.function_call.name)
